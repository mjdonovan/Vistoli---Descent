...
\xver{3.5 Equivalences of fibered categories}
\xver{3.6 Objects as fibcats and the 2-Yoneda}
\iver{3.7 The functors of arrows of a fibcat}
\iver{3.8 Equivariant objects in fibcats}
%%%%%%%%
\xver{4   STACKS}
...
\begin{shaded}
The definition of a sheaf on a site is the same as the one we're all used to --- either a unique gluing condition, or equivalently an equalizer diagram.
\end{shaded}
...
\begin{shaded}Suppose that $\cU = \{U_{i} \to U\}$ is a set of arrows, and $F \colon \cC\op \arr \catset$ a functor. We define $F\cU$ as the following equaliser. It comes equipped with a map (which I write as `$\textup{split}_\calU$') from $FU$, induced by the restrictions $FU \arr FU_i$:
...
We would like this to give us a functor from $\calC$ to the category of categories, but we made too many choices. Instead, given a cleavage of $\calF\to\calC$, we obtain a \emph{contavariant lax 2-functor} from $\calC$ to the 2-category of categories (a.k.a.\ a \textbf{pseudo-functor}). That is, 
\begin{itemize}\squishlist
...
but the latter category need not be a set, only an equivalence relation.
\end{shaded}
...
\hfil
\begin{shaded}
We would like to give an equivalence of categories:
\[
   \hom_{\cC}\bigl((\cC/X), \cF\bigr)
   \simeq \cF(X),
   \]
whenever $\calF$ is a fibered category over $\calC$. We describe the functors visually:
\[\xymatrix@R=0cm{
&%r2c1
&%r2c2
&%r2c3
&%r2c4
F(\id_X)\makebox[0cm][l]{\,$\in\calF(X)$}\ar[dd]^-{\alpha_{\id_X}}\\%r2c5
(\calC/X)
\xtwocell[rr]{}^F_{G}{{\alpha}}&%r1c1
&%r1c2
\calF&%r1c3
\mapsto&%r1c4
\\%r1c5
...
&%r2c3
&%r2c4
G(\id_X)\makebox[0cm][l]{\,$\in\calF(X)$}
}\qquad\qquad  \]
The first functor is straightforward. For the second, we give in the next diagram the definition on \emph{objects}. [Recall that the pullback $\psi^*\xi$ is defined up to isomorphism in $(\calF/\xi)$.]
   \[
   ...
   & \psi^*\xi\ar@{|->}[dd]\ar[r]
   &\xi\ar@{|->}[dd] \\
   U\ar[rd]_f\ar@/^/[rrd]|(.505)\hole^>>>>>\phi&&&\mapsfrom& U\ar[rd]_f\ar@/^/[rrd]^\phi\\
   & V \ar[r]_\psi
   & X&&   & V \ar[r]_\psi
   & X
   }
...
we identify $X$, $h_X$ and $(\calC/X)$ together, writing $X$ for $(\calC/X)$.

One finds that if $\cF \arr \cC$ is a fibered category:
\begin{itemize}\squishlist
\item $\cF$ is representable \Iff $\cF$ is fibered in groupoids, and there is an object $U$ of $\cC$ and an object $\xi$ of $\cF(U)$, such that for any object $\rho$ of $\cF$ there exists a unique arrow $\rho \arr \xi$ in $\cF$.
\item There exists a canonically defined split fibered category $\widetilde{\cF} \arr \cC$ and an equivalence of fibered categories of $\widetilde{\cF}$ with $\cF$.
\end{itemize}
